...
BASE_RESULTS = '%{corpus}_%{representation}_%{layers}_%{domain}'

def generate_command(representation, corpus, language, domain=nil, layers=nil)
  cmd = BASE_CMD
  cmd_hash = {representation: representation, corpus: corpus, language: language}
...
REPRESENTATIONS.product(CORPORA, VECTOR_DOMAIN, LAYERS).each do
  |representation, corpus, domain, layers|

  next if (representation == 'word_window' and not domain) or (representation != 'word_window' and domain)

  cmd = generate_command(representation, corpus, LANGUAGE[corpus.to_sym], domain, layers)
  STDERR.puts cmd
